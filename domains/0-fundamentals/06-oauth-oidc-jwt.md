...
Every OAuth exchange has three parties that are easy to conflate: the **resource owner** (the user), the **client** (the app requesting access — Keycloak calls it a client, Entra calls it an app registration), and the **authorization server** (Keycloak/Entra: authenticates the user and issues tokens). A fourth, the **resource server** (the API being called), trusts those tokens by validating them offline against the authorization server's published signing keys (or, for opaque tokens, via an introspection call) rather than round-tripping every request back to it. The **authorization code** grant is the one every other grant type is a variation or restriction of, so trace it hop by hop for a confidential web app:

1. **Authorization request.** The client redirects the browser to the authorization server's `/authorize` endpoint with `response_type=code`, `client_id`, `redirect_uri`, `scope`, a random `state` (CSRF protection — the client checks this comes back unchanged), an OIDC `nonce` (bound into the returned ID token so a replayed token is detectable), and — for PKCE — a `code_challenge` derived from a client-generated secret (`code_verifier`).
