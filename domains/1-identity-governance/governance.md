...
```
Both spellings mean the same thing — "produce one collection entry per set of bindings that satisfies this body" — and both are what `deny[msg] { … }`-shaped rules in the spec are pointing at. Use `_` when you'll never reference the iteration variable again (as above, iterating `input.parameters.labels`); use `some k` when the body references that key again, as the guardrail rule does with `k` (declaring two variables — `some k, v` — is for when you need the *value* alongside the key, not for reusing `k`). The Part B rule you're about to write is a partial-object rule in the older dialect — the `violation[{"msg": msg}] { … }` block above *is* the shape, with `required`/`provided` swapped for whatever your Constraint's parameters are.
...
What you need to act on the measure: `kubescape scan framework <name>` (e.g. `nsa`) produces a **compliance percentage** and a per-control breakdown against that framework — the open-source analogue of a Defender **secure score**. That percentage is **technical-control coverage, not a formal certification**: a 90% score means 90% of the framework's checked controls pass right now, not that an auditor has certified you against the standard. The full **mechanics** — which frameworks Kubescape ships, how the percentage and risk/priority score are computed and severity-weighted, and the report/output formats (`--format pdf|html|json|sarif|junit`) — are taught canonically under `vuln-compliance` in [`domains/4-posture-monitoring/vulnerability-posture.md`](../4-posture-monitoring/vulnerability-posture.md#produce-compliance-and-secure-score-style-reports); read it there for the full treatment. Here the point is the *governance application* — the measure you act on with policy.
