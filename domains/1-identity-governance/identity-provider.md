...
Domain 1, subsection 1 (`d1-idp`). Keycloak is the open-source stand-in for Microsoft Entra ID: an OIDC/SAML identity provider that owns your realms, users, groups, clients, authentication flows, MFA, brokering, and consent. Everything an SC-500 candidate has to reason about on Entra — how apps trust a token, how MFA is enforced, how conditional access decides, how OAuth consent is granted — has a concrete, inspectable equivalent here that you deploy and break yourself. Primary labs: [d1-keycloak-sso-mfa](../../labs/d1-keycloak-sso-mfa.md) and [d1-keycloak-conditional-access](../../labs/d1-keycloak-conditional-access.md); the lab-infra component is [`lab-infra/identity`](../../lab-infra/identity/) (Keycloak on the kind cluster). This note names grant types and decodes tokens without re-deriving how they work — if the authorization-code flow, PKCE, or JWT claims (`iss`/`aud`/`sub`/`exp`) aren't already second nature, read [`0-fundamentals/06-oauth-oidc-jwt.md`](../0-fundamentals/06-oauth-oidc-jwt.md) first.
...
A Keycloak **client** is an application's identity in a realm — the merged equivalent of an Entra **app registration** (the app definition) and **enterprise application / service principal** (the per-tenant instance). Each client has a `client_id`, a protocol (OIDC or SAML), a client-authentication mode, and a set of enabled OAuth grant types (called *capability config*) — see [`0-fundamentals/06-oauth-oidc-jwt.md`](../0-fundamentals/06-oauth-oidc-jwt.md) if the flows behind these toggles aren't already familiar:
