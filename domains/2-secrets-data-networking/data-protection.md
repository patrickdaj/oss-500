...
A Kubernetes Secret is only **base64-encoded** in etcd (see [fundamentals](../0-fundamentals/02-kubernetes.md#how-the-cluster-is-wired-and-where-its-soft), which also covers why etcd — as the store of *all* cluster state, reachable only through the API server — is the highest-value data-at-rest target in the cluster) — anyone with the etcd data file or a backup reads every secret in cleartext. **Encryption at rest** fixes this: the kube-apiserver encrypts resources (notably `secrets`) before writing them to etcd, using an **EncryptionConfiguration** passed via `--encryption-provider-config`. Providers, in ascending strength: `identity` (none), `aesgcm`/`aescbc`/`secretbox` (local key in the config file), and `kms` (v2) which delegates to an **external KMS** — and **Vault's transit engine** (`key-transit`) can be that KMS, so the data key that wraps etcd secrets lives in Vault, never on the API-server disk. This is the OSS realization of a **customer-managed key (CMK)**.
...
Getting that `--encryption-provider-config` flag onto a real apiserver means editing `/etc/kubernetes/manifests/kube-apiserver.yaml` on the control-plane node itself — kubeadm/kind run the API server as a **static pod**, a manifest the **kubelet** watches directly on local disk, not through the API server (which is exactly why this works at all: you can't `kubectl edit` the thing that *is* the API server). Save the file and the kubelet notices the change, tears down the old apiserver container, and starts a new one with your flag — no `kubectl apply`, no scheduler involved. The risk is real: a typo in the flag, a missing `volumeMount` for the key file, or a bad path, and the new apiserver container never comes up healthy — and because the apiserver is what's down, there is no `kubectl` path out of a self-inflicted outage. Recovery is `docker exec` (or `crictl`) straight into the node, not `kubectl`: restore the manifest to its last-known-good contents (keeping a copy before you touch it *is* the safety net) and save it — the kubelet's watch loop picks up the reverted file and restarts the apiserver again automatically, the same mechanism that applied your edit in the first place.
