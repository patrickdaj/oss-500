...
Vocabulary bridge before the snippet below — Domain 4's `obs-traces` teaches this in full (Collector, backends, sampling), but you need the four terms now, not weeks from now: a **span** is one timed unit of work (here, one LLM call); a **trace** is the tree of spans belonging to one request, correlated by a shared trace ID; the **`traceparent`** header (`00-<trace-id>-<span-id>-01`) is how that trace ID propagates across service hops so the spans link up into one trace instead of fragmenting; and **`gen_ai.*`** is OpenTelemetry's semantic-convention attribute namespace — a standard set of key names attached to a span so any backend can parse "this span was an LLM call, this model, this many tokens" without vendor-specific glue.
