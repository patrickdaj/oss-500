...
Primary lab: [d3-runtime-detection](../../labs/d3-runtime-detection.md). Lab-infra component: [`lab-infra/runtime`](../../lab-infra/runtime/) (Falco + Tetragon + Falcosidekick, deployed into the deliberately `privileged` `oss500-security` namespace because kernel instrumentation needs host access). The SC-500 analog is **Microsoft Defender for Containers** — specifically its runtime threat detection sensor, which does the same syscall/eBPF-based behavioral detection on AKS. This subsection reasons about hook points, the verifier, kprobe-vs-LSM, and CO-RE without re-deriving them — if those aren't already familiar, read [`0-fundamentals/07-ebpf-fundamentals.md`](../0-fundamentals/07-ebpf-fundamentals.md) first. It also assumes the **CRI/containerd boundary** the kubelet hands off to — where Falco and Tetragon actually hook — covered in the cluster-internals primer in [`0-fundamentals/02-kubernetes.md`](../0-fundamentals/02-kubernetes.md#how-the-cluster-is-wired-and-where-its-soft); the optional enrichment lab [`enrichment-kubelet-attack-surface`](../../labs/enrichment-kubelet-attack-surface.md) connects that boundary forward to what's built here.
