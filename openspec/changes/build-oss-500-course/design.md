...
**D2a — Terraform automation is a documented path, not shipped code.** The taught default stays the `up.sh`/`down.sh` wrappers over Helm/kubectl/Compose (D2). But Terraform can drive the whole stack and is itself `gov-iac` practice, so `lab-infra/README.md` documents the recipe: a cloud-agnostic `helm` + `kubernetes` "stack" module invoked by either a local-kind root (`tehcyx/kind`) or an Azure root (`azurerm` AKS/VM), with Docker Compose appliances via `kreuzwerker/docker`. The load-bearing property is that the in-cluster layer is byte-identical across kind and AKS — only the cluster module swaps — which is the portable-IaC lesson SC-500 tests. *Decision (user, this change):* document the path; do not ship `.tf` roots now. *Rationale:* keeps manifests legible as study material while making the "automate it all with Terraform, local or Azure" story explicit and actionable; the `.tf` layer can be added later without disturbing the content.
