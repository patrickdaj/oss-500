...
- [ ] **[0.5h] git + Terraform foundation** — read [domains/0-fundamentals/05-git-iac-foundation.md](../domains/0-fundamentals/05-git-iac-foundation.md) *first*: the git snapshot model (working tree/index/repo, branches, remotes, GitOps) and Terraform's write→plan→apply loop with state + locking. It's the foundation under today's applied kind/Helm work, and the ZTNA labs are Terraform-automated — get the plumbing straight before it's in your way.
- [ ] **[1h] Your first `terraform apply`** — write the ~10-line `main.tf` from the "Your first `terraform apply`" section of [05-git-iac-foundation.md](../domains/0-fundamentals/05-git-iac-foundation.md): create a namespace on the `kind-oss500` cluster with `init` → read the `plan` diff → `apply` → inspect `terraform.tfstate` → `destroy`. This is the blank-page HCL rep the Terraform-automated ZTNA labs assume you've already done.
