...
- [ ] **[0.5h] Bring up cert-manager again** — `cd lab-infra/certs && ./up.sh`. Day 4's teardown uninstalled cert-manager (CRDs included), which deleted every ClusterIssuer/Certificate — including the `oss500-ca-issuer` you hand-built that day. This re-ships the `ca-issuer` chain fresh; [d2-ingress-waf](../labs/d2-ingress-waf.md) targets that shipped issuer, not the one from Day 4.
