...
Notes live in [`domains/6-agentic-zero-trust/`](../domains/6-agentic-zero-trust/); labs in [`labs/`](../labs/); the agent, MCP server, and OPA policies in [`lab-infra/agentic/`](../lab-infra/agentic/). **This phase reuses, it doesn't rebuild:** the agentic lab-infra reuses the Keycloak realm and SPIRE trust domain from Phase 1 (`lab-infra/identity`) and the OPA engine from Phase 3 — an agent is just a new registration entry and a new token-exchange client on the identity plane you already run.
...
- [ ] **[2.5h] Lab — two identities, one agent** — [d6-identity](../labs/d6-identity.md) with [`lab-infra/agentic`](../lab-infra/agentic/): register the agent's SVID on the reused SPIRE server, then exchange a user token for a scoped `read` token on the reused Keycloak. **Observable: a token minted for `read` cannot call the write tool (audience/scope mismatch, refused at the resource), and an expired token is refused 401 — the authority is bounded and evaporates.**
...
- [ ] **[1h] Full teardown check** — bring the agentic stack down; confirm the reused identity/OPA plane is in the state you want for Review.
